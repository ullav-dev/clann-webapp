...
export default function EditPersonPage() {
  ...
  const api = useApi();
  const t = useTranslations("editPerson");
  ...
  if (loading) return <div className="py-20 text-center text-stone-400">{t("loading")}</div>;
  if (!person) return <div className="py-20 text-center text-red-600">{t("notFound")}</div>;

  return (
    <div className="max-w-2xl mx-auto">
      <div className="mb-8">
        <Link href={`/persons/${id}`} className="text-sm text-stone-500 hover:text-stone-700 transition-colors">{t("backTo", { name: fullName(person) })}</Link>
        <h1 className="text-3xl font-bold text-stone-800 mt-2">{t("title")}</h1>
      </div>
