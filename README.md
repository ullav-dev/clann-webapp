...
> **Note:** Use plain `API_URL` / `AUTH_URL` / `DAM_URL`, **not** `NEXT_PUBLIC_*` variants. `NEXT_PUBLIC_*` variables are inlined at build time and cannot be set at runtime. The defaults already point to the correct Docker service names, so these vars are only needed if your service names differ.
