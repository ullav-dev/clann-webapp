...
- **Relationships** — link people as Father, Mother, Sibling (Brother/Sister), or Spouse with a smart modal that filters the person list by sex (Father/Brother → males only, Mother/Sister → females only). Adding a **sibling** supports multi-select so several siblings can be linked in one step, and automatically inherits the root person's parents to all new siblings. Setting a **Father or Mother** automatically discovers that parent's other children and links them as full or half siblings — Clann checks whether each existing child shares the same other parent to determine whether the relationship is Biological or Half. Parent relationships support a **Relationship nature** selector: Biological (default) or Adopted. Siblings support Biological, Half, and Adopted. Step and foster relationships are not stored — step-siblings are derivable from spouse edges, and foster carries no genealogical weight. Spouse relationships carry optional **from** and **to** dates, editable inline in the Relationships tab.
