...
- `IDLE_TIMEOUT_MS` — idle session timeout in milliseconds (default: `3600000` = 1 hour). **Runtime** plain env var: the server layout injects it into `window.__ENV__` and `AuthContext` reads it via `resolveIdleMs`, so it can be changed with a container restart (`.env.prod`) — no rebuild. Invalid/blank values fall back to the default and a **2-minute floor** (`MIN_IDLE_MS`) prevents an accidentally tiny value from logging users straight out. The legacy build-time `NEXT_PUBLIC_IDLE_TIMEOUT_MS` is still read as a fallback but is deprecated — prefer `IDLE_TIMEOUT_MS`. To exercise the warning modal set `IDLE_TIMEOUT_MS=120000` (warning at 60 s, logout at 120 s).
